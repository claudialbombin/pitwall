...
class RaceSimulator:
        ...
        for i in range(n_runs):
            ...
            if use_antithetic:
                ...
                result_anti = self._simulate_race(
                    antithetic_rng,
                    starting_position,
                    starting_compound,  # type: ignore[arg-type]
                )
    ...
    @staticmethod
    def _baseline_heuristic(car: CarState, sc_active: bool, total_laps: int) -> Action:
        """
    ...
    def summarise(self, results: list[RaceResult]) -> dict:
        ...
        summary = {
            ...
            "mean_first_pit_lap": round(float(np.mean(first_pit_laps)), 1)
            if first_pit_laps
            else None,
            "std_first_pit_lap": round(float(np.std(first_pit_laps)), 1)
            if first_pit_laps
            else None,
            "p25_first_pit_lap": round(float(np.percentile(first_pit_laps, 25)), 0)
            if first_pit_laps
            else None,
            "p75_first_pit_lap": round(float(np.percentile(first_pit_laps, 75)), 0)
            if first_pit_laps
            else None,
        }
        return summary
